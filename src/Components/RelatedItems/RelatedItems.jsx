...
var RelatedItems = ({ current, setCurrent }) => {
  ...
  function compareToCurrent(item) {
    var newComparison = [
      ...
      [current.default_price, "Price", item.default_price],
    ]
    var mapFeatures = (features) => {
      var featureObj = {}
      features.forEach(
        (feature) => (featureObj[feature.feature] = feature.value)
      )
      return featureObj
    }
    ...
    for (var i = 0; i < combinedKeys.length; i++) {
      newComparison.push([
        currentFeatures[combinedKeys[i]],
        combinedKeys[i],
        comparatorFeatures[combinedKeys[i]],
      ])
    }
  ...
  function getProducts(suffix) {
    return axios
      .get(`/products${suffix === undefined ? "" : "/" + suffix}`)
      .catch((err) => {
        console.log(err)
      })
  }
  ...
  function setCurrentById(id) {
    getProducts(id).then((res) => setCurrent(res.data))
  }
  ...
  function getRelated() {
    ...
    return getProducts(`${current.id}/related`)
      ...
      .then((results) => {
        return Promise.all(
          results.map((result) => getProducts(`${result.id}/styles`))
        )
      })
      .then((results) => {
        results = results.map((result) => {
